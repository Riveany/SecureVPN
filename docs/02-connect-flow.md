...
`Connect-VPN` ([AutoVPN.ps1:1198](../AutoVPN.ps1)) runs a fixed seven-step
...
`Start-SVPClient` ([AutoVPN.ps1:445](../AutoVPN.ps1)) launches the executable at
...
Followed by a 2-second wait (`DoEvents-Sleep`, which despite the name is now a plain cancellable sleep on the worker thread).

### Step 2 — Find the Login window

`Find-SVPLoginWindow` ([AutoVPN.ps1:472](../AutoVPN.ps1)) polls every 500 ms for
...
`Click-LoginButton` ([AutoVPN.ps1:507](../AutoVPN.ps1)) clicks control ID
**1018** via `Win32::ClickButton`. It does nothing else — no restore, no raise,
no focus change.

It previously called `ShowWindow(SW_RESTORE)` first, with a 300 ms wait. That
was removed: `BM_CLICK` is delivered to the control's message queue and does not
require the window to be visible, restored, or focused. Verified by minimising
the login window deliberately and clicking Login anyway — the Security Alert
appeared, and the foreground window did not change.

Restoring the window only pulled SVPClient in front of whatever the user was
doing, and put a clickable window on screen mid-sequence.

Followed by a 2-second cancellable wait.

### Step 4 — Handle the Security Alert

`Handle-SecurityAlert` ([AutoVPN.ps1:528](../AutoVPN.ps1)) polls for up to 10
...
Followed by a 1-second cancellable wait.

### Step 5 — Find the Authentication window

`Find-AuthWindow` ([AutoVPN.ps1:597](../AutoVPN.ps1)) polls for up to 20 seconds
...
`Fill-AuthForm` ([AutoVPN.ps1:633](../AutoVPN.ps1)) enumerates all child controls
...
([AutoVPN.ps1:742](../AutoVPN.ps1)) polls for up to 30 seconds. It succeeds as
...
`Test-VpnConnectedNow` ([AutoVPN.ps1:715](../AutoVPN.ps1)) is the single source
...
`Disconnect-VPN` ([AutoVPN.ps1:1219](../AutoVPN.ps1)) does not use the client's
own disconnect button. It kills the process.

The reason is stated in a comment at [AutoVPN.ps1:1055](../AutoVPN.ps1):
...
Connect-VPN
  ...
  ├─ 3. Click-LoginButton            (BM_CLICK id 1018, no window raise)

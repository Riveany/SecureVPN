...
([AutoVPN.ps1:264](../AutoVPN.ps1)) if absent or unparseable. Written by
...
| `minimize_to_tray` | bool | `true` | When true, minimising the window hides it instead ([AutoVPN.ps1:1510](../AutoVPN.ps1)). |
| `credential_file` | string | `vpn_cred.dat` | Path to the credential store. Resolved relative to `$Script:ScriptDir` unless already rooted ([AutoVPN.ps1:290](../AutoVPN.ps1)). |
...
Created by `Save-VpnCredential` ([AutoVPN.ps1:298](../AutoVPN.ps1)). Gitignored.
...
([AutoVPN.ps1:313](../AutoVPN.ps1)). The connect sequence then aborts and directs
...
1. Inside `Load-VpnCredential`, which converts the `SecureString` back to a plain
   ...
   ([AutoVPN.ps1:313](../AutoVPN.ps1)) and returns it in a hashtable.
...
after use ([AutoVPN.ps1:888](../AutoVPN.ps1)). This shortens the exposure window
...
`Reset-VpnCredential` ([AutoVPN.ps1:339](../AutoVPN.ps1)) removes the file with
`Remove-Item -Force`. It is a plain delete, not a secure wipe.

## Auto-start shortcut

`Set-AutoStart` ([AutoVPN.ps1:1300](../AutoVPN.ps1)) creates or removes
...
([AutoVPN.ps1:1462](../AutoVPN.ps1)). Its colour does not change with connection

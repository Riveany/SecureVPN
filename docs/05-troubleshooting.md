...
([AutoVPN.ps1:352](../AutoVPN.ps1)); when launched through
...
later ([AutoVPN.ps1:396](../AutoVPN.ps1)).
...
3. Was the client slower than 15 seconds to show its window? The timeout is a
   parameter default at [AutoVPN.ps1:400](../AutoVPN.ps1).
...
after this warning ([AutoVPN.ps1:510](../AutoVPN.ps1)). Read those lines — they
...
`Edit` controls ([AutoVPN.ps1:561](../AutoVPN.ps1)).
...
credentials were submitted ([AutoVPN.ps1:686](../AutoVPN.ps1)). This is the
...
`InterfaceDescription` ([AutoVPN.ps1:647](../AutoVPN.ps1)). A different adapter
...
([AutoVPN.ps1:662](../AutoVPN.ps1)). It fires during the interval between the
...
The short version: the connect sequence runs on the UI thread and pumps the
message loop while waiting, so tray-menu clicks and the Settings dialog run
nested inside it.

## The application freezes completely with no log output

Distinct from the stall above. If SVPClient raises a modal dialog in response to
a click, a synchronous `SendMessage` does not return until that dialog closes —
freezing the UI thread with no timeout.

Clicks now use `PostMessage` and text operations use `SendMessageTimeout`, so
this should no longer occur. If it does, an un-migrated `SendMessage` call is the
...
`Load-Config` runs once, at the start of `Main` ([AutoVPN.ps1:1533](../AutoVPN.ps1)).
...
2. The script requires STA. The entry point relaunches itself if the apartment
   state is wrong ([AutoVPN.ps1:1583](../AutoVPN.ps1)); if that relaunch loops,
...
trace ([AutoVPN.ps1:1602](../AutoVPN.ps1)).
